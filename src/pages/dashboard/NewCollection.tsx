...
const FormSchema = z.object({
  ...
  fields: z
    .array(z.string())
    .min(1, { message: "At least one field must be selected" }),
});
    ...
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      <DialogTrigger asChild>
        <div className="flex gap-4">
          <div className="ml-1">
            {" "}
            <Plus id="plus-icon" data-testid="plus-icon" size={32} />
          </div>
          <div>
            {" "}
            <span className="hidden sm:block mr-4">New Collection</span>
          </div>
        </div>
      </DialogTrigger>
